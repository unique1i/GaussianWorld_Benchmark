...
def process_dataset(data_root, splits_dir, split_file, dry_run):
    ...
    print(
        f"[{dataset}] Scenes to {'simulate removal' if dry_run else 'delete'} ({len(to_remove)}):"
    )
...
def main():
    ...
    splits = [
        "holicity_mini_val.txt",
        "matterport3d_mini_test.txt",
        "scannet_mini_val.txt",
        "scannetpp_mini_val.txt",
    ]
    if not splits:
