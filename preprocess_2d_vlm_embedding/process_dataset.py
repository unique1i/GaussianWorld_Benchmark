...
def main():
    parser = argparse.ArgumentParser(description="Batch run preprocess.py on multiple scenes.")
    parser.add_argument(
        "--txt_file", 
        type=str, 
        required=True, 
    ...
    parser.add_argument(
        "--dataset_name", 
        type=str, 
        required=True, help="scannet or scannetpp"
    )
    parser.add_argument(
        "--save_folder", 
        type=str, 
    ...
    print(f"Processing scenes from line {args.start_idx} to "
    ...
    for idx, scene_id in enumerate(selected_scenes):
        if args.dataset_name == "scannet":
            ...
            elif os.path.exists(os.path.join(args.dataset_folder, "scans_test", scene_id)):
            ...
            dataset_path,
            "--save_folder", 
            args.save_folder if args.save_folder else os.path.join(args.dataset_folder, "language_features"),
            "--zipped"
            ]
        elif args.dataset_name == "scannetpp":
            # split ["data", "sem_test"]
            if os.path.exists(os.path.join(args.dataset_folder, "data", scene_id, "dslr")):
                dataset_path = os.path.join(args.dataset_folder, "data", scene_id, "dslr")
            elif os.path.exists(os.path.join(args.dataset_folder, "sem_test", scene_id, "dslr")):
                dataset_path = os.path.join(args.dataset_folder, "sem_test", scene_id, "dslr")
            else:
                raise FileNotFoundError(f"Scene folder not found: {scene_id} at {args.dataset_folder}")
            cmd = [
            "python",
            ...
            dataset_path,
            "--save_folder", 
            args.save_folder if args.save_folder else os.path.join(args.dataset_folder, "language_features_siglip2"),
            "--resolution",
        ...
        print(f"Processing scene {scene_id} ({idx + 1}/{len(selected_scenes)}), {dataset_path}")
