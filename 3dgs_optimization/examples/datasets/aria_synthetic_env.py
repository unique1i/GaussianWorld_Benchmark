...
class AriaSyntheticEnvsDataset(Dataset):
    def __init__(
        ...
        self.image_zip = None
        self.depth_zip = None
        self.depth_scale = 1000  # Depth is in mm
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        if self.load_depth and depth_rel_path is not None:
            depth_tensor = self._load_depth(depth_rel_path, zipped, intrinsics)
            if self.resize and (
    ...
    def _init_zip_refs(self):
        ...
        """
        if self.images_zip is not None:
            return  # Already initialized

        try:
            self.images_zip = zipfile.ZipFile(
                self.scene_path / "rgb_undistorted.zip", "r"
            )
            self.depth_zip = zipfile.ZipFile(
                self.scene_path / "depth_undistorted.zip", "r"
            )
    ...
    def _load_image(self, image_rel_path: str, zipped: bool) -> Image.Image:
        ...
        if zipped:
            # Load image from zip
            if self.images_zip is None:
                self._init_zip_refs()
            try:
                with self.images_zip.open(image_rel_path) as img_file:
    ...
    def _load_depth(self, depth_rel_path: str, zipped: bool, intrinsics: None) -> torch.Tensor:
        ...
        def _raydepth_to_zdepth_mm(
            ...
            """
            Convert depth measured along the camera ray to z-axis depth (both in mm).
            Optionally supply `scale` = 1 + (u-cx)^2/fx^2 + (v-cy)^2/fy^2 to reuse
            pre-computed broadcast matrix.
            """
            depth_f = depth_mm.astype(np.float32)
            if scale is None:
                h, w = depth_f.shape
                vs = (np.arange(w) - cx) / fx
                us = (np.arange(h) - cy) / fy
        ...
        if zipped:
            ...
            if self.depth_zip is None:
                self._init_zip_refs()
            try:
                with self.depth_zip.open(depth_rel_path) as depth_file:
                    # use cv2 to read the depth image unchanged
                    depth_mm = cv2.imdecode(
                        np.frombuffer(depth_file.read(), np.uint8),
                        cv2.IMREAD_UNCHANGED,
                    ).astype(np.float32)
            except Exception as e:
                raise f"Error loading depth image {depth_rel_path}: {e}"
        else:
