...
class BlenderDatasetZipped(torch.utils.data.Dataset):
    def __init__(
        ...
        self.fx = self.meta["fx"]
        self.fy = self.meta["fy"]
        ...
        if self.meta["zipped"]:
            self.image_zip_path = self.data_root / "color_interval.zip"
            self.depth_zip_path = self.data_root / "depth_interval.zip"
        ...
        for frame in self.meta["frames"]:
            self.image_paths.append(frame["file_path"] + ".jpg")
    ...
    def _load_depth(self, image_name: str):
        if self.depth_zip_path:
            ...
            img_path = self.data_root / image_name.replace("jpg", "png")
            img = Image.open(img_path)
