...
if __name__ == "__main__":
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for idx, feature in enumerate(train_loader):
            ...
            # get mask of 0 feature
            # print("data.shape", data.shape)
            l2_norm = torch.norm(data, p=2, dim=1)
            # print("l2_norm", l2_norm.shape)
            mask = l2_norm > 0.1
            # print("valid mask", mask.sum(), mask.shape)
        ...
        if epoch > num_epochs - 20:
            ...
            for idx, feature in enumerate(test_loader):
                ...
                eval_loss_cossine += cos_loss(outputs, data) * len(feature)

                eval_loss += loss * len(feature)
            eval_loss = eval_loss / len(train_dataset)
            print("eval_loss:{:.8f}".format(eval_loss))
            print(
                "eval_loss_cossine:{:.8f}".format(
                    eval_loss_cossine / len(train_dataset)
                )
            )
            if eval_loss < best_eval_loss:
                best_eval_loss = eval_loss
                best_epoch = epoch
                print("save best model to", f"ckpt/{args.dataset_name}/best_ckpt.pth")
                torch.save(
                    model.state_dict(), f"ckpt/{args.dataset_name}/best_ckpt.pth"
                )

            if epoch % 10 == 0:
                torch.save(
                    model.state_dict(), f"ckpt/{args.dataset_name}/{epoch}_ckpt.pth"
                )
