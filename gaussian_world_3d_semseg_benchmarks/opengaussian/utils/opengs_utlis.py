...
def calculate_pairwise_distances(tensor1, tensor2, metric=None):
    ...
    if metric == "l2":
        return None, torch.sqrt(
            (tensor1 - tensor2).pow(2).sum(dim=2)
        )  # Result is [m, n]
...
def calculate_iou(masks1, masks2, base=None):
    ...
    if base == "former":
        ...
        union = (masks1_expanded | masks2_expanded).float().sum(
            dim=(2, 3)
        ) + 1e-6  # [m, n]
...
def get_SAM_mask_and_feat(
    gt_sam_mask, level=3, filter_th=50, original_mask_feat=None, sample_mask=False
):
    """
    input:
    ...
    one_hot = F.one_hot(
        mask_id.type(torch.int64), num_classes=int(instance_num.item() + 1)
    )
    # bool mask [num+1, H, W]
    ...
    # update mask id
    mask_id = torch.argmax(
        mask_bool, dim=0
    )  # [H, W] The ID of the pixels after filtering is 0
...
def process_in_chunks(masks_expanded, masked_feats, mean_per_channel, chunk_size=5):
    ...
    for i in range(0, masks_expanded.size(0), chunk_size):
        ...
        for j in range(0, masks_expanded.size(1), chunk_size):
            ...
            result[i:end_i, j:end_j] = torch.where(
                chunk_mask.bool(),
                chunk_feats - chunk_mean,
                torch.zeros_like(chunk_feats),
            )
    return result


def calculate_variance_in_chunks(masked_for_variance, mask_counts, chunk_size=5):
    variance_per_channel = torch.zeros(
        masked_for_variance.size(0),
        masked_for_variance.size(1),
        device=masked_for_variance.device,
    )
    for i in range(0, masked_for_variance.size(0), chunk_size):
        ...
        for j in range(0, masked_for_variance.size(1), chunk_size):
            ...
            chunk_variance = (chunk_masked_for_variance**2).sum(
                dim=[2, 3]
            ) / mask_counts[i:end_i, j:end_j]
...
def mask_feature_mean(feat_map, gt_masks, image_mask=None, return_var=False):
    ...
    feat_expanded = feat_map.unsqueeze(0).expand(
        num_mask, *feat_map.shape
    )  # [num_mask, C, H, W]
    masks_expanded = gt_masks.unsqueeze(1).expand(
        -1, feat_map.shape[0], -1, -1
    )  # [num_mask, C, H, W]
    if image_mask is not None:  # image level mask
        image_mask_expanded = image_mask.unsqueeze(0).expand(
            num_mask, feat_map.shape[0], -1, -1
        )

    # average features within each mask
    if image_mask is not None:
        masked_feats = (
            feat_expanded * masks_expanded.float() * image_mask_expanded.float()
        )
        ...
        masked_feats = ele_multip_in_chunks(
            feat_expanded, masks_expanded, chunk_size=5
        )  # in chuck to avoid OOM
    ...
    if not return_var:
        ...
        masked_for_variance = process_in_chunks(
            masks_expanded, masked_feats, mean_per_channel, chunk_size=5
        )  # in chunk to avoid OOM

        # variance_per_channel = (masked_for_variance ** 2).sum(dim=[2, 3]) / mask_counts    # [num_mask, 6]
        variance_per_channel = calculate_variance_in_chunks(
            masked_for_variance, mask_counts, chunk_size=5
        )  # in chuck to avoid OOM
