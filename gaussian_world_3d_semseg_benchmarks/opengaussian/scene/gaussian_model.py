...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int):
        ...
        self._ins_feat = torch.empty(
            0
        )  # Continuous instance features before quantization
        self._ins_feat_q = torch.empty(
            0
        )  # Discrete instance features after quantization
    ...
    def update_learning_rate(self, iteration, root_start, leaf_start):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if param_group["name"] == "ins_feat":
                if (
                    iteration > root_start and iteration <= leaf_start
                ):  # TODO: update lr
    ...
    def save_ply(self, path, save_q=[]):
        ...
        r, g, b = (
            vis_color[:, 0].reshape(-1, 1),
            vis_color[:, 1].reshape(-1, 1),
            vis_color[:, 2].reshape(-1, 1),
        )
        ...
        attributes = np.concatenate(
            (xyz, normals, ins_feat, f_dc, f_rest, opacities, scale, rotation, r, g, b),
            axis=1,
        )
    ...
    def load_ply_gs_only(self, path):
        ...
        ins_feat = torch.rand((xyz.shape[0], 6), dtype=torch.float, device="cuda")
        ...
        self._ins_feat = nn.Parameter(
            torch.tensor(ins_feat, dtype=torch.float, device="cuda").requires_grad_(
                True
            )
        )
    ...
    def load_ply(self, path):
        ...
        ins_feat = np.stack(
            (
                np.asarray(plydata.elements[0]["ins_feat_r"]),
                np.asarray(plydata.elements[0]["ins_feat_g"]),
                np.asarray(plydata.elements[0]["ins_feat_b"]),
                np.asarray(plydata.elements[0]["ins_feat_r2"]),
                np.asarray(plydata.elements[0]["ins_feat_g2"]),
                np.asarray(plydata.elements[0]["ins_feat_b2"]),
            ),
            axis=1,
        )
        opacities = np.asarray(plydata.elements[0]["opacity"])[..., np.newaxis]
