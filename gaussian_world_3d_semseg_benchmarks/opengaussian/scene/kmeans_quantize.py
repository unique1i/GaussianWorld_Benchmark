...
class Quantize_kMeans:
    def __init__(
        self, num_clusters=64, num_leaf_clusters=10, num_iters=10, dim=9, dim_leaf=6
    ):
    ...
    def get_dist(self, x, y, mode="sq_euclidean"):
        ...
        if mode == "sq_euclidean_chunk":
            ...
            for i in range(np.ceil(x.shape[0] / step).astype(int)):
                dist.append(
                    torch.cdist(
                        x[(i * step) : (i + 1) * step, :].unsqueeze(0), y.unsqueeze(0)
                    )[0]
                )
    ...
    def update_centers(self, feat, mode="root", selected_leaf=-1):
        ...
        feat = torch.cat(
            [feat, torch.zeros_like(feat[:1]).cuda()], 0
        )  # [num_pts+1, dim]
        centers = torch.sum(
            feat[self.cluster_ids, :].reshape(num_clusters, self.max_cnt, -1), dim=1
        )  # [num_clusters, vec_dim]
    ...
    def update_centers_(self, feat, cluster_mask=None, nn_index=None, avg=False):
        ...
        centers = (
            cluster_mask.T @ feat
        )  # [1w, num_cluster] * [1w, dim] -> [num_cluster, dim]
        # if avg:
            ...
            all_ids.append(
                torch.cat(
                    [
                        ...
                        -1
                        * torch.ones(
                            (self.max_cnt - len(cur_cluster_ids)), dtype=torch.long
                        ).cuda(),
                    ]
                )
            )
    ...
    def cluster_assign(self, feat, feat_scaled=None, mode="root", selected_leaf=-1):
        ...
        if len(self.leaf_centers) == 0 and mode == "leaf":
            ...
            self.leaf_centers = feat[
                torch.randperm(feat.shape[0])[
                    : self.num_clusters * self.leaf_num_clusters + 1
                ],
                :,
            ]
            self.leaf_cls_ids = (
                torch.ones(feat.shape[0]).to(torch.int64).cuda()
                * self.num_clusters
                * self.leaf_num_clusters
            )
        ...
        if mode == "root":
            ...
            tmp_centers = torch.zeros_like(self.leaf_centers)[
                : self.leaf_num_clusters, :
            ]
            counts = (
                torch.zeros(self.leaf_num_clusters, dtype=torch.float32).cuda() + 1e-6
            )
            start_id = selected_leaf * self.leaf_num_clusters
            end_id = (
                selected_leaf * self.leaf_num_clusters + self.iLeafSubNum[selected_leaf]
            )
        for iteration in range(self.num_kmeans_iters):
            ...
            if chunk:
                ...
                if mode == "root":
                    while True:
                        dist = self.get_dist(
                            feat[i * chunk : (i + 1) * chunk, :], self.centers
                        )
                        curr_nn_index = torch.argmin(dist, dim=-1)  # [1W]
                        # Assign a single cluster when distance to multiple clusters is same
                        dist = F.one_hot(curr_nn_index, self.num_clusters).type(
                            torch.float32
                        )  # [1W, 512]
                        curr_centers = self.update_centers_(
                            feat[i * chunk : (i + 1) * chunk, :],
                            dist,
                            curr_nn_index,
                            avg=False,
                        )  # [512, 45]
                        ...
                        if self.nn_index == None:
                            ...
                            self.nn_index = torch.cat(
                                (self.nn_index, curr_nn_index), dim=0
                            )
                        i += 1
                    ...
                    for idx_c in range(self.num_clusters):
                        ...
                        dist = self.get_dist(
                            feat[selected_pts], self.leaf_centers[start_id:end_id]
                        )
                        curr_nn_index = torch.argmin(dist, dim=-1)  # [1W]
                        dist = F.one_hot(curr_nn_index, self.leaf_num_clusters).type(
                            torch.float32
                        )  # [1W, 10]
                        curr_centers = self.update_centers_(
                            feat[selected_pts], dist, curr_nn_index, avg=False
                        )  # [512, 45]
                        counts += dist.detach().sum(0) + 1e-6  # [512]
                        tmp_centers += curr_centers
            ...
            if mode == "root":
                ...
                self.leaf_centers[start_id : start_id + self.leaf_num_clusters] = (
                    tmp_centers / counts.unsqueeze(-1)
                )
            # Reinitialize to 0
            tmp_centers[tmp_centers != 0] = 0.0
        ...
        if chunk:
            ...
            if mode == "root":
                while True:
                    dist = self.get_dist(
                        feat_scaled[i * chunk : (i + 1) * chunk, :], self.centers
                    )
    ...
    def forward(
        ...
        if assign:
            self.cluster_assign(
                feat, mode=mode, selected_leaf=selected_leaf
            )  # gaussian._ins_feat
        else:
            self.update_centers(
                feat, mode=mode, selected_leaf=selected_leaf
        ...
        if mode == "root":
            ...
            centers = self.leaf_centers
            vec_dim = self.leaf_vec_dim
        sampled_centers = torch.gather(
            centers, 0, self.nn_index.unsqueeze(-1).repeat(1, vec_dim)
        )
        ...
        gaussian._ins_feat_q = (
            gaussian._ins_feat - gaussian._ins_feat.detach() + sampled_centers[:, :6]
        )

    def replace_with_centers(self, gaussian):
        deg = gaussian._features_rest.shape[1]
        sampled_centers = torch.gather(
            self.centers, 0, self.nn_index.unsqueeze(-1).repeat(1, self.vec_dim)
        )
        gaussian._features_rest = (
            gaussian._features_rest
            - gaussian._features_rest.detach()
            + sampled_centers.reshape(-1, deg, 3)
        )
