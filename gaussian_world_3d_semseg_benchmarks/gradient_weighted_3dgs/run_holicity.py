...
if __name__ == "__main__":
    ...
    for scene in validation_set:
        # scene = scene.split('/')[-1]
        if args.rescale == 0:
            potential_results_dir = os.path.join(result_root, scene, 'features_lseg_512_512.pt')
        ...
        if os.path.exists(potential_results_dir) and os.path.exists(potential_xyz_dir):
            ...
            continue   
        print(scene)
        os.system("python backproject_ply_holicity.py --data_root_path {} --ply_root_path {} --results_root_dir {} \
