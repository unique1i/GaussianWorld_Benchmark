...
def get_arguments():
    ...
    parser.add_argument('--split', type=str, default='/splits/scannetpp_mini_val.txt', help='Split name')
    parser.add_argument('--rescale', type=int, default=0, help='rescale custom')
    parser.add_argument('--result_root', type=str, default='./results', help='result root')
    parser.add_argument(
        "--data_root_path",
        type=str,
...
if __name__ == "__main__":
    ...
    for scene in validation_set:
        scene = scene.split('/')[-1]
        if args.rescale == 0:
            potential_results_dir = os.path.join(result_root, scene, 'features_lseg_584_876.pt')
        elif args.rescale == 1:
            potential_results_dir = os.path.join(result_root, scene, 'features_lseg_480_640.pt')
        elif args.rescale == 2:
            potential_results_dir = os.path.join(result_root, scene, 'features_lseg_240_320.pt')

        if os.path.exists(potential_results_dir):
            print("Found potential results dir: ", potential_results_dir)
            continue   
        print(scene)
        os.system("python backproject_ply_scannetpp.py --data_root_path {} --ply_root_path {} --results_root_dir {} \
