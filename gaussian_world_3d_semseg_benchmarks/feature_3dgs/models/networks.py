...
def sf_linear_transformation(input_tensor, output_dim=512):
    ...
    transformation_matrix = (
        torch.randn(input_tensor.shape[0], input_tensor.shape[-1], output_dim)
...
def fmap_linear_transformation(input_tensor, output_dim):
    ...
    torch.manual_seed(seed)
    transformation_matrix = (
        torch.randn(input_tensor.shape[0], output_dim).requires_grad_(False).cuda()
    )  # (16, 512)
    reshaped_tensor = input_tensor.view(
        input_tensor.shape[0], -1
    )  # (16,360,480) -> (16, 360*480)
    transformed_tensor = torch.matmul(
        transformation_matrix.T, reshaped_tensor
    )  # (512, 16) (16, 360*480) -> (512, 360*480)
    output_tensor = transformed_tensor.view(
        -1, input_tensor.shape[1], input_tensor.shape[2]
    )
...
class MLP_decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = (
            x.permute(1, 2, 0).contiguous().view(-1, input_dim)
        )  # (16,48,64)->(48,64,16)->(48*64,16)
