...
def feature_map(feature):
    ...
    covariance_matrix = (
        f_samples_centered.T @ f_samples_centered / (f_samples_centered.shape[0] - 1)
    )
    ...
    vis_feature = (
        fmap.permute(0, 2, 3, 1).reshape(-1, fmap.shape[1]) - mean
    ) @ top_eig_vectors
    vis_feature = (
        vis_feature - feature_pca_postprocess_sub
    ) / feature_pca_postprocess_div
    vis_feature = (
        vis_feature.clamp(0.0, 1.0).float().reshape((fmap.shape[2], fmap.shape[3], 3))
    )
