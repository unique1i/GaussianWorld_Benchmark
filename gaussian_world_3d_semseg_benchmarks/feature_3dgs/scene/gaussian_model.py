...
class GaussianModel:
    ...
    def create_from_pcd(
        ...
        self._semantic_feature = (
            torch.zeros(fused_point_cloud.shape[0], semantic_feature_size, 1)
            .float()
            .cuda()
        )
        ...
        self._semantic_feature = nn.Parameter(
            self._semantic_feature.transpose(1, 2).contiguous().requires_grad_(True)
        )
    ...
    def construct_list_of_attributes(self):
        ...
        for i in range(
            self._semantic_feature.shape[1] * self._semantic_feature.shape[2]
        ):
    ...
    def load_gs_ply_only(self, path, semantic_feature_size, speedup=True):
        ...
        #     features_dc = (features_dc - 0.5) / 0.28209479177387814
        ...
        if len(extra_f_names) == 3 * (self.max_sh_degree + 1) ** 2 - 3:
            ...
            features_extra = np.zeros(
                (xyz.shape[0], 3 * (self.max_sh_degree + 1) ** 2 - 3)
            )
            features_extra = features_extra.reshape(
                (features_extra.shape[0], 3, (self.max_sh_degree + 1) ** 2 - 1)
            )
        ...
        self._semantic_feature = (
            torch.zeros(self._xyz.shape[0], semantic_feature_size, 1).float().cuda()
        )
        self._semantic_feature = nn.Parameter(
            self._semantic_feature.transpose(1, 2).contiguous().requires_grad_(True)
        )
    ...
    def load_ply(self, path):
        ...
        count = sum(
            1
            for name in plydata.elements[0].data.dtype.names
            if name.startswith("semantic_")
        )
        semantic_feature = np.stack(
            [np.asarray(plydata.elements[0][f"semantic_{i}"]) for i in range(count)],
            axis=1,
        )
        semantic_feature = np.expand_dims(semantic_feature, axis=-1)
