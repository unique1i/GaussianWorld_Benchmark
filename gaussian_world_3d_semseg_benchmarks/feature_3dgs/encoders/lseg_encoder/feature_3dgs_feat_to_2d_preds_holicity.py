...
def main():
    ...
    for scene_name in scene_ids:
        for split in ['train',]:
            ...
            scene_folder = os.path.join(args.results_root_dir, scene_name)
       
            feature_3dgs_path = f'feature-3dgs_Qi/output/holicity/{scene_name}'
            ...
            with torch.no_grad():
                text = clip.tokenize(holicity_label_prompt)
            ...
            feat_files = sorted(glob.glob(os.path.join(feature_3dgs_path, split, f'ours_{iteration}', test_feat_name, "*.pt")))

            print("feat_files: ", len(feat_files), 'in', os.path.join(feature_3dgs_path, split, f'ours_{iteration}', test_feat_name))
            assert len(feat_files) > 0, f"No feature files found in {os.path.join(feature_3dgs_path, split, f'ours_{iteration}', test_feat_name)}"
