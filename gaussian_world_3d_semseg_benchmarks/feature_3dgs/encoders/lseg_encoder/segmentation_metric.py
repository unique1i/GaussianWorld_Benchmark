...
def calculate_iou(teacher, student, num_classes):
    ...
    unique_labels, counts = np.unique(
        np.concatenate((teacher, student)), return_counts=True
    )
...
def calculate_iou_mask(gt, teacher, student, num_classes):
    ...
    unique_labels, counts = np.unique(
        np.concatenate((gt, teacher, student)), return_counts=True
    )
    sorted_indices = np.argsort(-counts)
    sorted_labels = unique_labels[sorted_indices]
...
###
class FeatureImageFolderLoader(
    enc_ds.ADE20KSegmentation
):  # (torch.utils.data.Dataset):
    def __init__(
        self,
        student_feature_root,
        ...
        if len(self.student_features) == 0:
            raise (
                RuntimeError(
                    "Found 0 prediction features in subfolders of: \
                "
        ...
        if len(self.teacher_features) == 0:
            raise (
                RuntimeError(
                    "Found 0 ground truth features in subfolders of: \
                "
                    + self.gt_label_root
                    + "\n"
                )
    ...
    def __getitem__(self, index):
        ...
        return (
            ...
            teacher_feature,
            image,
            os.path.basename(self.student_features[index]),
        )
...
def get_gttest_folder_features(fea_folder):
    def numerical_sort_key(filename):
        ...
        ]  # returns a list of numbers for multi-level sorting
...
def get_feature_image_dataset(
    student_feature_path, teacher_feature_path, image_path, eval_mode, **kwargs
):
    if (
        os.path.isdir(student_feature_path)
        and os.path.isdir(teacher_feature_path)
        and os.path.isdir(image_path)
    ):
        return FeatureImageFolderLoader(
            student_feature_path,
...
def test(args):
    ...
    testset = get_feature_image_dataset(
        args.student_feature_dir,
        args.teacher_feature_dir,
        args.test_rgb_dir,
        args.eval_mode,
    ...
    for i, (student_features, teacher_features, images, dst) in enumerate(tbar):
        ...
        with torch.no_grad():
            ...
            for student_feature in student_features:
                ...
                image_feature = student_feature.permute(1, 2, 0).reshape(
                    -1, feashape[0]
                )  # (172800, 512)
                ...
                )  # torch.Size([172800, 150]) logit_scale
                logits_per_image = logits_per_image.view(
                    feashape[1], feashape[2], -1
                ).permute(2, 0, 1)
                # print("################################### logits_per_image: ", logits_per_image.shape) # torch.Size([150, 360, 480])
                pred_image_text_feature = logits_per_image[
                    None
                ]  # torch.Size([1, 150, 360, 480])
            ...
            for teacher_feature in teacher_features:
                ...
                # feature = F.normalize(feature.float(), dim=0).half()
                image_feature = teacher_feature.permute(1, 2, 0).reshape(
                    -1, feashape[0]
                )  # (172800, 512)
                image_feature = image_feature / image_feature.norm(
                    dim=-1, keepdim=True
                ...
                # print("################################### logits_per_image: ", logits_per_image.shape) # torch.Size([150, 360, 480])
                gt_image_text_feature = logits_per_image[
                    None
                ]  # torch.Size([1, 150, 360, 480])
            ...
            pred_predicts = [
                testset.make_pred(
                    torch.max(pred_image_text_feature, 1)[1].cpu().numpy()
                )
        ...
        for pred_predict, gt_predict, impathp in zip(pred_predicts, gt_predicts, dst):
            ...
            gt_predict = F.interpolate(
                gt_predict.unsqueeze(0), size=(119, 159), mode="nearest"
            ).squeeze(0)
            gt_predict = gt_predict.long().numpy()  # Convert back to numpy array

            pred_predict = torch.from_numpy(pred_predict).float()
            pred_predict = F.interpolate(
                pred_predict.unsqueeze(0), size=(119, 159), mode="nearest"
            ).squeeze(0)
            ...
            print(
                f"the average accuracy is {accuracy_accum / count}, average iou is {iou_accum / count}"
            )
