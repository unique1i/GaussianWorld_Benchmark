...
class LSeg(BaseModel):
    ...
    def forward(self, x, labelset="", return_feature=False):
        ...
        layer_1, layer_2, layer_3, layer_4 = forward_vit(
            self.pretrained, x, preresize=self.preresize
        )
        ...
        if return_feature:
            ...
            # print("####################################### before normalize: ", image_features.shape) # torch.Size([1, 512, 240, 240])
            ###
            ...
            # print("####################################### after normalize: ", image_features.shape) # torch.Size([1, 512, 480, 480])
            return image_features  # torch.Size([1, 512, 480, 480])

        image_features = image_features.permute(0, 2, 3, 1).reshape(-1, self.out_c)
        # normalized features
        image_features = image_features / image_features.norm(
            dim=-1, keepdim=True
        )  # torch.Size([57600, 512])
        ...
        logits_per_image = (
            self.logit_scale * image_features.half() @ text_features.t()
        )  # torch.Size([57600, 150])
        out = (
            logits_per_image.float()
            .view(imshape[0], imshape[2], imshape[3], -1)
            .permute(0, 3, 1, 2)
        )  # torch.Size([1, 150, 240, 240])
        ...
        out = self.scratch.output_conv(out)  # torch.Size([1, 150, 480, 480])
