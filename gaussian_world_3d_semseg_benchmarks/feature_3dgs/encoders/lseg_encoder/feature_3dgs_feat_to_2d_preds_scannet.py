...
def get_arguments():
    ...
    parser.add_argument('--split', type=str, default='/splits/scannet_mini_val.txt', help='Split name')
...
def main():
    ...
    for scene_name in os.listdir('/usr/bmicnas02/data-biwi-01/qimaqi_data/workspace/neurips_2025/feature-3dgs_Qi/output/scannet'):
        for split in ['train', 'test']:
            ...
            ignore_mask_20 = np.array([name in ignore_classes for name in label_names_20], dtype=bool)
            ignore_mask_200 = np.array([name in ignore_classes for name in label_names_200], dtype=bool)
            ...
            for feat_path in tqdm(feat_files):
                ...
                print("pred_labels_20 shape: ", pred_labels_20.shape, pred_labels_20.min(), pred_labels_20.max())
                print("pred_labels_200 shape: ", pred_labels_200.shape, pred_labels_200.min(), pred_labels_200.max())


                # saving labels
                label_save_path = feat_path.replace("_fmap_CxHxW.pt", "_pred_labels.npy").replace(test_feat_name, "pred_labels_20")
                ...
                label_save_path = feat_path.replace("_fmap_CxHxW.pt", "_pred_labels.npy").replace(test_feat_name, "pred_labels_200")
                os.makedirs(os.path.dirname(label_save_path), exist_ok=True)
                print("save pred_labels to: ", label_save_path)
