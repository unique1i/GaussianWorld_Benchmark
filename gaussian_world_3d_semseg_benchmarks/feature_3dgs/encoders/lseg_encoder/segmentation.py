...
class Options:
    def __init__(self):
        ...
        parser.add_argument(
            ...
            default=0,
            metavar="N",
            help="dataloader threads",  ### default=16
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--weights",
            type=str,
            default="demo_e200.ckpt",
            help="checkpoint to test",  ### default=None
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            default=True,
            action="store_true",
            help="wider output head",  ### default=False
        )
        parser.add_argument(
            "--widehead_hr",
            default=False,
        ...
        ###
        parser.add_argument(
            "--iteration", type=int, required=True, help="Chosen number of iterations"
        )
...
class FeatureImageFolderLoader(
    ...
    def __init__(self, feature_root, image_root, transform=None):
        ...
        if len(self.features) == 0:
            raise (
                RuntimeError(
                    "Found 0 features in subfolders of: \
                "
                    + self.root
                    + "\n"
                )
...
def get_feature_image_dataset(feature_path, image_path, **kwargs):
    if os.path.isdir(feature_path) and os.path.isdir(image_path):
        return FeatureImageFolderLoader(
            feature_path, image_path, transform=kwargs["transform"]
        )
...
def test(args):
    ...
    for name in ["test", "train", "novel_views"]:
        ...
        image_path = os.path.join(
            input_path, "ours_{}".format(args.iteration), "renders"
        )
        feature_path = os.path.join(
            input_path, "ours_{}".format(args.iteration), "saved_feature"
        )
        ...
        for _, (features, images, dst) in enumerate(tbar):
            with torch.no_grad():
                if images[0].shape[-1] > w:
                    ...
                    images = [
                        ...
                        for image in images
                    ]  # images[0]: torch.Size([3, 1440, 1920])
                    ...
                    ]  ### features[0]: torch.Size([512, 1440, 1920])
                ...
                for feature in features:
                    ...
                    image_feature = feature.permute(1, 2, 0).reshape(
                        -1, feashape[0]
                    )  # (172800, 512)
                    ...
                    logits_per_image = logits_per_image.view(
                        ...
                        2, 0, 1
                    )  # torch.Size([150, 360, 480])
                    image_text_feature = logits_per_image[
                        None
            ...
            for predict, impath, img, fmap in zip(predicts, dst, images, features):
                ...
                Image.fromarray((vis.cpu().numpy() * 255).astype(np.uint8)).save(
                    os.path.join(output_path, outname + "_vis.png")
                )
                ...
                plt.savefig(
                    os.path.join(output_path, outname + "_legend.png"),
                    format="png",
                    dpi=300,
                    bbox_inches="tight",
                )
