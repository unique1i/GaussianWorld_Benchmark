...
def feature_visualize_saving(feature):
    ...
    del f_samples
    vis_feature = (
        fmap.permute(0, 2, 3, 1).reshape(-1, fmap.shape[1]) - feature_pca_mean[None, :]
...
def parse_edit_config_and_text_encoding(edit_config):
    ...
    if edit_config is not None:
        ...
        text_feature = clip_editor.encode_text(
            [obj.replace("_", " ") for obj in objects]
        )
...
def render_set(
    ...
    if edit_config != "no editing":
        ...
        edit_render_path = os.path.join(
            model_path,
            name,
            "ours_{}_{}_{}".format(
                iteration, next(iter(edit_dict["operations"])), target
            ),
            "renders",
        )
        edit_gts_path = os.path.join(
            model_path,
            name,
            "ours_{}_{}_{}".format(
                iteration, next(iter(edit_dict["operations"])), target
        ...
        edit_feature_map_path = os.path.join(
            model_path,
            name,
            "ours_{}_{}_{}".format(
                iteration, next(iter(edit_dict["operations"])), target
        ...
        edit_gt_feature_map_path = os.path.join(
            model_path,
            name,
            "ours_{}_{}_{}".format(
                iteration, next(iter(edit_dict["operations"])), target
        ...
        feature_map_path = os.path.join(
            model_path, name, "ours_{}".format(iteration), "feature_map"
        )
        gt_feature_map_path = os.path.join(
            model_path, name, "ours_{}".format(iteration), "gt_feature_map"
        )
        saved_feature_path = os.path.join(
            model_path, name, "ours_{}".format(iteration), "saved_feature"
        )
        # encoder_ckpt_path = os.path.join(model_path, "encoder_chkpnt{}.pth".format(iteration))
        decoder_ckpt_path = os.path.join(
            model_path, "decoder_chkpnt{}.pth".format(iteration)
        )
        depth_path = os.path.join(
            model_path, name, "ours_{}".format(iteration), "depth"
        )  ###
        depth_npy_path = os.path.join(
            model_path, name, "ours_{}".format(iteration), "depth_npy"
        )  ###
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        if edit_config != "no editing":
            render_pkg = render_edit(
                view, gaussians, pipeline, background, text_feature, edit_dict
            )
            gt = view.original_image[0:3, :, :]
            gt_feature_map = view.semantic_feature.cuda()
            torchvision.utils.save_image(
                render_pkg["render"],
                os.path.join(edit_render_path, "{0:05d}".format(idx) + ".png"),
            )
            torchvision.utils.save_image(
                gt, os.path.join(edit_gts_path, "{0:05d}".format(idx) + ".png")
            )
            ...
            Image.fromarray(
                ...
                os.path.join(
                    edit_feature_map_path, "{0:05d}".format(idx) + "_feature_vis.png"
                )
            )
            ...
            Image.fromarray(
                ...
                os.path.join(
                    edit_gt_feature_map_path, "{0:05d}".format(idx) + "_feature_vis.png"
                )
            )
            ...
            torchvision.utils.save_image(
                render_pkg["render"], os.path.join(render_path, image_name + ".png")
            )
            torchvision.utils.save_image(
            ...
            depth_npy = depth.cpu().numpy()
            np.save(os.path.join(depth_npy_path, image_name + ".npy"), depth_npy)
            ...
            output_path = os.path.join(depth_path, image_name + ".png")
            ...
            Image.fromarray(
                (feature_map_vis.cpu().numpy() * 255).astype(np.uint8)
            ).save(os.path.join(feature_map_path, image_name + "_feature_vis.png"))
            gt_feature_map_vis = feature_visualize_saving(gt_feature_map)
            ...
            Image.fromarray(
                (gt_feature_map_vis.cpu().numpy() * 255).astype(np.uint8)
            ).save(os.path.join(gt_feature_map_path, image_name + "_feature_vis.png"))
            ...
            torch.save(
                torch.tensor(feature_map).half(),
                os.path.join(saved_feature_path, image_name + "_fmap_CxHxW.pt"),
            )


def render_video(
    model_path, iteration, views, gaussians, pipeline, background, edit_config
):  ###
    ...
    final_video = cv2.VideoWriter(
        os.path.join(render_path, "final_video.mp4"), fourcc, 10, size
    )
    ...
    for idx, pose in enumerate(tqdm(render_poses, desc="Rendering progress")):
        ...
        view.full_proj_transform = (
            view.world_view_transform.unsqueeze(0).bmm(
                view.projection_matrix.unsqueeze(0)
            )
        ).squeeze(0)
        ...
        final_video.write(
            (rendering.permute(1, 2, 0).detach().cpu().numpy() * 255.0).astype(
                np.uint8
            )[..., ::-1]
        )
    final_video.release()
...
def render_novel_views(
    ...
    for idx, pose in enumerate(tqdm(poses, desc="Rendering progress")):
        view.world_view_transform = (
            torch.tensor(
                getWorld2View2(pose[:, :3], pose[:, 3], view.trans, view.scale)
            )
            .transpose(0, 1)
            .cuda()
        )
...
def render_novel_video(
    model_path, name, iteration, views, gaussians, pipeline, background, edit_config
):
