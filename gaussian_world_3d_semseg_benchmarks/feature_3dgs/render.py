...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, edit_config, speedup):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        if edit_config != "no editing":
            ...
            torchvision.utils.save_image(render_pkg["render"], os.path.join(render_path, image_name + ".png"))
            ...
            Image.fromarray((feature_map_vis.cpu().numpy() * 255).astype(np.uint8)).save(os.path.join(feature_map_path, image_name + "_feature_vis.png"))
            gt_feature_map_vis = feature_visualize_saving(gt_feature_map)
            ...
            Image.fromarray((gt_feature_map_vis.cpu().numpy() * 255).astype(np.uint8)).save(os.path.join(gt_feature_map_path, image_name + "_feature_vis.png"))
            ...
            torch.save(torch.tensor(feature_map).half(), os.path.join(saved_feature_path, image_name + "_fmap_CxHxW.pt"))
