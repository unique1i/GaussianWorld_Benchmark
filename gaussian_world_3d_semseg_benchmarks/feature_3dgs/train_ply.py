...
def training(
    ...
    if ply_path:
        gaussians.load_gs_ply_only(
            ply_path, semantic_feature_size=512, speedup=dataset.speedup
        )
        first_iter = 0
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if iteration % 100 == 0:
            ...
            gt_image_save = (
                (gt_image_save * 255).byte().permute(1, 2, 0).contiguous().cpu().numpy()
            )
            ...
            render_img_save.save(
                "./{}_render_image_{}.png".format(source_name, iteration)
            )
